...
function do_page_start($opts = array(), $child = false, $vue = false)
{
    ...
    if (isset($opts['enterprise'])) {
        if ($theme == "xi5" || $theme == "xi5dark"  || $theme == 'neptune' || $theme == "colorblind") {
...
?>
    <link rel="preload" href='<?php echo $base_url; ?>includes/fonts/MaterialSymbolsOutlined-100-700.woff2?<?php echo get_build_id(); ?>' as="font" type="font/woff2" crossorigin="anonymous">
    ...
    <script type='text/javascript' src='<?php echo $base_url; ?>includes/js/core.js?<?php echo get_build_id(); ?>'></script>
        <?php if ($theme == "xi5" || $theme == "xi5dark"  || $theme == 'neptune') { ?>
...
function neptune_centered_checkbox($text, $id, $name, $value, $checked_value, $disabled = false, $tooltip = "", $checkbox_value = "on")
{
    ob_start(); ?>   
    <div class="centered-checkbox">
        <input type="checkbox" class="" id="<?= $id; ?>" name="<?= $name; ?>" value="<?php echo $checkbox_value ?>" <?php echo is_checked($value, $checked_value); ?> <?php echo ($disabled ? 'disabled' : ''); ?>> 
