...
//  Copyright (c) 2010-2024 Nagios Enterprises, LLC
//
//  Authors:
...
function ccm_component_head_include($cbtype='', $args=null)
{
    ...
    if (strpos($_SERVER['PHP_SELF'], 'includes/components/ccm/') !== false || strpos($_SERVER['PHP_SELF'], 'includes/components/bulkmodifications/') !== false) {
        ...
        if(is_neptune()) {
            echo "<link rel='stylesheet' type='text/css' href='../../css/themes/neptune.css' />";
        }
...
// Requires Nagios XI version greater than 2024R1.1
...
function ccm_has_access_for($permission)
{
    ...
    // Check for ccm access
    if (!user_can_access_ccm($userid)) {
        return false;
    }

    if (in_array($permission, array('host', 'service')) && !is_authorized_to_configure_objects($userid)) {
        return false;
    }

    // Check for ccm access type
    if (get_user_meta($userid, 'ccm_access') != 2) {
        return true;
    }
    ...
    $ccm_access_list = get_user_meta($userid, 'ccm_access_list', array());
