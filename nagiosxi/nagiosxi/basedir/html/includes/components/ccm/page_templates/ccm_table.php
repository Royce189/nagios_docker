...
function ccm_table($args, $returnContent=array(0, ''), $page=1)
{
    ...
    if (ccm_has_access_for($type)) {
        $add_new_button = "<a class='btn btn-sm btn-primary vtop fl icon-in-btn' style='margin-right: 15px;' href='index.php?cmd=insert&type=".encode_form_valq($type)."&returnUrl=".urlencode($returnUrl)."'><span class='material-symbols-outlined md-400'>add</span> "._("Add New")."</a>";
        if(is_neptune()) {
            $add_new_button = "<a class='btn btn-sm btn-primary vtop fl neptune-ccm-table-new' style='margin-right: 15px;' href='index.php?cmd=insert&type=".encode_form_valq($type)."&returnUrl=".urlencode($returnUrl)."'><span class='material-symbols-outlined md-400'>add</span> "._("Add New")."</a>";
        }
    }
    ...
    {   
        ...
        if (is_neptune()) {
            if (ccm_has_access_for($type)) {
                $icons .= "<a class='action ccm-tt-bind' href='?cmd=modify&type=".encode_form_valq($type)."&id={$id}&page={$page}&returnUrl=".urlencode($returnUrl)."' title='"._('Edit')."'><span class='material-symbols-outlined icon-color-override ccm-table-action'>build</span></a>";
                $icons .= "<a><span class='action ccm-tt-bind' title='"._("Copy")."' onclick=\"actionPic('copy', '{$id}', '')\"><span class='material-symbols-outlined icon-color-override ccm-table-action'>content_copy</span></span></a>";
            }

            if (!$ccm_restricted) {
                $icons .= "<a><span class='action ccm-tt-bind' title='"._('View config')."' onclick=\"actionPic('download', '". addslashes($cfg_name)."', '{$_SESSION['domain']}')\"><span class='material-symbols-outlined icon-color-override ccm-table-action'>description</span></span></a>";
            }

            $icons .= "<a><span class='action ccm-tt-bind' title='"._('Relationships')."' onclick=\"show_relationship_popup('{$type}', '".addslashes($info_name)."', '{$id}', '".$_SESSION['token']."')\"><span class='material-symbols-outlined icon-color-override ccm-table-action'>group</span></span></a>";

            if (ccm_has_access_for($type)) {
                $icons .= "<a><span class='action ccm-tt-bind' title='Delete' onclick=\"actionPic('delete', '{$id}', '".addslashes($name)."')\"><span class='material-symbols-outlined icon-color-override ccm-table-action'>delete</span></span></a>";
            }
        } else {
            if (ccm_has_access_for($type)) {
                $icons .= "<a class='action ccm-tt-bind' href='?cmd=modify&type=".encode_form_valq($type)."&id={$id}&page={$page}&returnUrl=".urlencode($returnUrl)."' title='"._('Edit')."'><span class='material-symbols-outlined icon-color-override ccm-table-action md-300'>build</span></a>";
                $icons .= "<a><span class='action ccm-tt-bind' title='"._("Copy")."' onclick=\"actionPic('copy', '{$id}', '')\"><span class='material-symbols-outlined icon-color-override ccm-table-action md-300'>content_copy</span></span></a>";
            }

            if (!$ccm_restricted) {
                $icons .= "<a><span class='action ccm-tt-bind' title='"._('View config')."' onclick=\"actionPic('download', '". addslashes($cfg_name)."', '{$_SESSION['domain']}')\"><span class='material-symbols-outlined icon-color-override ccm-table-action md-300'>description</span></span></a>";
            }

            $icons .= "<a><span class='action ccm-tt-bind' title='"._('Relationships')."' onclick=\"show_relationship_popup('{$type}', '".addslashes($info_name)."', '{$id}', '".$_SESSION['token']."')\"><span class='material-symbols-outlined icon-color-override ccm-table-action md-300'>group</span></span></a>";

            if (ccm_has_access_for($type)) {
                $icons .= "<a><span class='action ccm-tt-bind' title='Delete' onclick=\"actionPic('delete', '{$id}', '".addslashes($name)."')\"><span class='material-symbols-outlined icon-color-override ccm-table-action md-300'>delete</span></span></a>";
            }
        }
