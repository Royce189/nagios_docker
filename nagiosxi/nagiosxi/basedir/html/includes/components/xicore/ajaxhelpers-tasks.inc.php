...
function xicore_ajax_get_admin_tasks_html($args = null)
{
    ...
    if ($opt != 1) {
        if ($neptune) {
            ...
            $setupoutput .= "<span class='black-blue-400'>" . _('Configure basic settings for your system.') . '</span>';
        }
        else {
    ...
    if ($opt != 1) {
        if ($neptune) {
            ...
            $setupoutput .= "<span class='black-blue-400'>" . _('Configure email settings for your  system') . '</span>';
        }
        else {
    ...
    $output .= "<tr><td>";
    if ($neptune) {
        $output.= "<span class='black-blue-400'>" . _('Add or modify items to be monitored') . '</span>';
        ...
        $output .= "<span class='black-blue-400'>" . sprintf(_('Setup new users with access to %s'), get_product_name()) . '</span>';
...
function xicore_ajax_get_getting_started_html($args = null)
{
    ...
    if ($neptune) {
        $output .= "<span class='black-blue-400'>" . _('Change your account password and general preferences') . '</span>';
        ...
        $output .= "<span class='black-blue-400'>" . _('Change how and when you receive alert notifications') . '</span>';
        ...
        $output .= "<span class='black-blue-400'>" . _('Add or modify items to be monitored with easy-to-use wizards') . '</span>';
        $output .= "<a class='btn btn-admin' href='" . $config_base . "' target='_top'><span class='material-symbols-outlined md-18 md-400' title=" . _('Configure your monitoring setup') . ">computer</span>" . _('Configure your monitoring setup') . "</a>";
    }
    else {
        $output .= "<ul>";
    ...
    if (!custom_branding()) {
        ...
        $output .= "<tr><td>";
        if ($neptune) {
            $output .= "<span class='black-blue-400'>" . _('Learn more about XI and its capabilities') . '</span>';
            $output .= "<a class='btn btn-admin' href='" . $product_url . "' target='_blank'><span class='material-symbols-outlined md-18 md-400' title=" . _('Learn about XI') . ">school</span>" . _('Learn about XI') . "</a>";
            $output .= "<span class='black-blue-400'>" . _('Stay informed on the latest updates and happenings for XI') . '</span>';
            $output .= "<a class='btn btn-admin' href='" . $product_url . "#stayinformed' target='_blank'><span class='material-symbols-outlined md-18 md-400' title=" . _('Signup for XI news') . ">newspaper</span>" . _('Signup for XI news') . "</a>";
        }
        else {
            $output .= "<ul>";
