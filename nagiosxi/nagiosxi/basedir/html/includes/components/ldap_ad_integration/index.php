...
function route_request()
{
    ...
    {        
        ...
        case "finish":
            ...
            $count = create_nagios_users($users, $preferences, $security);
            flash_message(_("Successfully added").' '.$count.' '._("users"), 'success');
            do_page_start(array("page_title" => _("LDAP / Active Directory Users")), true);
            finish_page($count);
            break;
...
function landing_page()
{
    ...
    .user-list { margin: 20px; height: calc(100% - 90px); position: relative; overflow-y: auto; }
    .user-list li { }
    .user-list li label { cursor: pointer; }
    .sub-list li span { padding-left: 2.5px; }
    ...
    .user-toggle-show-dn { font-size: 11px; vertical-align: baseline; cursor: pointer; margin-left: 2px; }
    .ad-ldap-container { width: 32%; min-width: 200px; }
    .btn > label, .btn > label > input { cursor: pointer; }
    </style>
    
    <p class="neptune-subtext"><?php echo _("Select the users you would like to give access to Nagios XI via LDAP/AD authentication. You will be able to set user-specific permissions on the next page."); ?></p>
    <div class="neptune-section-spacer"></div>
    <h4 class="neptune-form-spacer"><?php echo _("Select Users to Import from LDAP/AD"); ?></h4>

    <div id="selected-users">
        <span class="num-users">0</span> <?php echo _("users selected for import"); ?><span class="users"></span>
    </div>

    <div class="ad-ldap-wrapper">
        <div id="root" class="ad-ldap-container"></div>
        <div id="view" style="min-width: 600px; display: table-cell; vertical-align: top; ">
    ...
    function filter_base_organizational_units(path) {
        let base_dn = '<?php echo $adldap->getBaseDn()?>';
        let raw_organizational_units = base_dn.split(',');
        let base_organizational_units = [];

        raw_organizational_units.forEach((designation) => {
            if (designation.includes("OU=")) {
                // Only grab the the right side of the equals
                let organizational_unit = designation.split("=")[1];
                base_organizational_units.push(organizational_unit);
            }
        })

        if (path instanceof Array) {
            path = path.filter(organizational_unit => !base_organizational_units.includes(organizational_unit));
        }
    ...
    function grab_ad_obj(target_form, type, path, folder)
    {
        ...
        // The path cannot contain the same organizational units (OUs) as the base DN
        let finalPath = filter_base_organizational_units(path);

        if (already_loaded == 0) {
            $.ajax({
                type: "POST",
                url: "index.php",
                data: { cmd: "display_nav_window", object_type: type, target_path: finalPath },
                success: function(response) {
                    ...
                    grab_ad_users(target_form, type, finalPath);
                },
...
<?php
        ...
        echo neptune_subtext(_("Log into your LDAP / Active Directory administrator or privileged account to be able to import users.") );
?>
    <form method="post" style="margin: 30px 0;">
<?php
        echo neptune_text('', 'ad_username', 'username', encode_form_val($username), '', '', '', 'placeholder="Username"');

        echo neptune_password('', 'ad_password', 'password', encode_form_val($password), '', 'placeholder="Password"');
    }
    ...
    foreach ($servers as $server) {
        ...
        if (!is_neptune()) {
?>
                <option value="<?= $server['id'] ?>" <?= $selected?>><?= ldap_ad_display_type($server['conn_method']) ?> - <?= encode_form_val($display_servers) ?></option>
<?php
        } else {
            $conn_method_options[$server['id']] = ldap_ad_display_type($server['conn_method']).' - '.encode_form_val($display_servers);
        }
    }
...
<?php
        ...
        echo neptune_select('', '', 'server_id', $conn_method_options, $server_id);
    }
?>
        <input type="hidden" name="cmd" value="landing_page">
        <button type="submit" class="btn btn-sm btn-primary btn-icon-and-text" name="next"><?= _("Next") ?> <i class="material-symbols-outlined md-20 md-400">chevron_right</i></button>
    </form>

    <a class="text-medium" href="<?= get_component_url_base("ldap_ad_integration") ?>/manage.php"><?= _("Manage Authentication Servers") ?></a>
<?php
}
        ...
        foreach ($array_to_enum as $obj) {
            if (is_array($obj)) {
                ...
                if (in_array($stype, $containers)) {
                    ...
                    if ($stype == "container" || $type == "nsContainer") { $image = "folder"; }
                    if ($stype == "organizationalunit") { $image = "folder_open"; }
                    ?>
                    <li>
                        <span class="ad-folder" data-path='<?php echo $path; ?>' data-type="<?php echo $type; ?>">
                            <span class="table-icon material-symbols-outlined md-pointer md-20 md-400" style=""><?php echo $image ?></span>
                            <?php echo $dn; ?>
                        </span>
                    </li>
                    <?php
...
function display_users($array_to_enum, $location, $type, $selected, $search="")
{
            ...
            foreach ($array_to_enum as $username)
            {
                ...
                <li>
                    <label style="font-weight: normal; display: inline-flex; align-items: center; gap: 5px;">
                        <input type="checkbox" class="ad-checkbox" style="margin: 0 5px 0 0; vertical-align: middle;" data-username="'.$username.'" value="'.$obj.'" '.$checked.'>
                        <span class="table-icon material-symbols-outlined md-pointer md-20 md-400" alt="'._("Add New User").'" title="'._("Add New User").'" style="">person</span>
                        '.$displayname.' ('.$username.')
                    </label>
                    <i class="material-symbols-outlined md-400 material-icon-va user-toggle-show-dn" title="'._("Show full DN (distinguished name)").'">add_box</i>
            ...
            foreach ($array_to_enum as $obj)
            {
                ...
                if (is_array($obj)) {
                    ...
                    if (in_array($stype, $units)) {
                        ...
                        $display_image = '';
                        if (!empty($image)) {
                            $display_image = '<span class="table-icon material-symbols-outlined md-pointer md-20 md-400" alt="'._("User").'" title="'._("User").'" style="">' . $image .'</span>';
                        }
                        ...
                        <li>
                            ...
                            <i class="material-symbols-outlined md-400 material-icon-va user-toggle-show-dn" title="'._("Show full DN (destinguished name)").'">add_box</i>
                            <div class="user-dn hide">DN: '.$dn.'</div>
                        </li>';
                    }
                }
    ...
    <div style="margin: 20px 0 0 20px" class="form-inline">
        <div class="fl align-items-center-flex">
            ...
            <button id="ad-search-btn" class="btn btn-sm btn-default btn-alignment" ><span class="material-symbols-outlined md-pointer md-20 md-400">search</span></button>
            <?php if (!empty($search)) { ?>
            <button id="ad-search-clear-btn" class="btn btn-sm btn-default btn-alignment" style="<?php if (!is_neptune()) { echo "margin-left: 5px;"; } ?>"><span class="material-symbols-outlined md-pointer md-20 md-400">close</span></button>
            <?php } ?>
        </div>
        <div class="fr">
...
function show_user_options($users, $revise)
{
    ...
    <p class="neptune-subtext"><?php echo _('Set the preferences and security settings for all users that will be imported. You can also edit multiple user\'s preferences/security settings at once by checking the users you want to edit and selecting the action from the dropdown.'); ?></p>
    ...
    $(document).ready(function() {
        ...
        function generate_popup(eid, etype) {
            ...
            if (etype == "preferences") {
                ...
                specific_content = '<div id="popup_data" style="min-width: 550px;"><p>'+$('.preferences-form').clone().html()+'</p>';
            } else if (etype == "security") {
                edit_title = "<?php echo encode_form_val(_('Security Settings')); ?>";
                specific_content = '<div id="popup_data" data-eid="'+eid+'" style="min-width: 550px;"><p>'+$('.security-form').clone().html()+'</p>';
            }

            var content = '<div id="popup_header"><b>'+edit_title+'</b></div>';
            content += specific_content;
            content += '<input type="hidden" value="'+eid+'" id="eid"><input type="hidden" value="'+etype+'" id="etype"><button type="button" class="e-save btn btn-sm btn-primary neptune-btn">' + "<?php echo encode_form_val(_('Save')); ?>" + '</button> <button type="button" class="e-cancel btn btn-sm btn-default neptune-btn">' + "<?php echo encode_form_val(_('Cancel')); ?>" + '</button></div>';
        ...
        $('.import').click(function() {
            ...
            $.post("<?php echo get_component_url_base('ldap_ad_integration'); ?>/ajax.php", { cmd: 'getxiusers' }, function(users) {
                ...
                if (!errors) {
                    ...
                    $('.import').prop('disabled', false).html('<?php echo encode_form_valq(_("Import")); ?> <i class="material-symbols-outlined md-20 md-400">chevron_right</i>');
                }
                ...
                <tr>
                    ...
                    <td>
                        <select name="theme" class="theme form-control">
                            ...
                            <option value="colorblind"<?php if ($theme == 'colorblind') { echo " selected"; } ?>><?php echo _("Colorblind"); ?></option>
                        </select>
                    </td>
                </tr>
                <tr>
            ...
            <tbody>
                <tr>
                    <td>
                        ...
                        <i tabindex="1" class="material-symbols-outlined md-20 pop md-middle" data-content="<b><?php echo _('User'); ?></b> - <?php echo _('Can only see hosts and services they are a contact of by default. View only, no configuration access by defualt.'); ?><br><br><b><?php echo _('Admin'); ?></b> - <?php echo _('Access to all objects by default and can control, access, and configure the entire Nagios XI system and has access to the admin section.'); ?>">help</i>
                    </td>
                    <td>
                ...
                <tr class="user-setting">
                    <td>
                        ...
                        <i tabindex="2" class="material-symbols-outlined md-20 pop md-middle" data-content="<?php echo _('Allows a user to view all host and services that are applied in the system.'); ?><br><br><i><?php echo _('Note: User does not need to be a contact of an object to see it with this enabled.'); ?></i>">help</i>
                    </td>
                    <td>
                ...
                <tr class="user-setting">
                    <td>
                        ...
                        <i tabindex="3" class="material-symbols-outlined md-20 pop md-middle" data-content="<?php echo _('Allows the user do the following on hosts and services they have access to'); ?>: <ul style='margin: 0; padding: 10px 0 5px 20px;'><li><?php echo _('Acknowledge problems'); ?></li><li><?php echo _('Schedule downtime'); ?></li><li><?php echo _('Toggle notifications'); ?></li><li><?php echo _('Force checks on all objects'); ?></li></ul>">help</i>
                    </td>
                    <td>
                ...
                <tr class="user-setting">
                    <td>
                        ...
                        <i tabindex="4" class="material-symbols-outlined md-20 pop md-middle" data-content="<?php echo _('Allows a user to be able to access the following for hosts and services:'); ?><ul style='margin: 0; padding: 10px 0 5px 20px;'><li><?php echo _('Run Configuration Wizards'); ?></li><li><?php echo _('Delete from detail page'); ?></li><li><?php echo _('Re-configure from detail page'); ?></li</ul>">help</i>
                    </td>
                    <td>
                ...
                <tr class="user-setting">
                    <td>
                        ...
                        <i tabindex="5" class="material-symbols-outlined md-20 pop md-middle" data-content="<?php echo _('Allows the editing of check command in the re-configure host/service page.<br><br>Shows advanced tab and commands in host/service detail pages.<br><br>Allows setting host parents during wizards and re-configure on host detail page.'); ?>">help</i>
                    </td>
                    <td>
                ...
                <tr class="user-setting">
                    <td>
                        ...
                        <i tabindex="6" class="material-symbols-outlined md-20 pop md-middle" data-content="<?php echo _('Allows access to the Monitoring Process section in the main page.'); ?><br><br><?php echo _('Allows managing Nagios Core process such as starting, stopping, restarting, and changing process settings.'); ?><br><br><?php echo _('Allows access to the Event Log.'); ?>">help</i>
                    </td>
                    <td>
                ...
                <tr class="user-setting">
                    <td>
                        ...
                        <i tabindex="7" class="material-symbols-outlined md-20 pop md-middle" data-content="<?php echo _('Restrict the user to have read-only access.'); ?><br><br><i><?php echo _('Note: This overwrites the following'); ?>:<ul style='margin: 0; padding: 10px 0 5px 20px;'><li><?php echo _('Can control hosts and services'); ?></li><li><?php echo _('Can configure hosts and services'); ?></li><li><?php echo _('Can access advanced features'); ?></li></ul></i>">help</i>
                    </td>
                    <td>
                ...
                <tr class="user-setting">
                    <td>
                        ...
                        <i tabindex="8" class="material-symbols-outlined md-20 pop md-middle" data-content="<?php echo _('Allow access to use the API and integrated help documentation.'); ?><br><br><i><?php echo _('Note: Users can only access the objects API endpoint.'); ?></i>">help</i>
                    </td>
                    <td>
                ...
                <tr class="user-setting"><td></td><td></td></tr>
                    ...
                    <td>
                        ...
                        <i tabindex="9" class="material-symbols-outlined md-20 pop md-middle" data-content="<?php echo _('Allow user to view and access the CCM.'); ?><br><br><b><?php echo _('None'); ?></b> - <?php echo _('Cannot use or view the CCM.'); ?><br><br><b><?php echo _('Login'); ?></b> - <?php echo _('Can view CCM links and must log in with a CCM user account.'); ?><br><br><b><?php echo _('Limited'); ?></b> - <?php echo _('Integrated CCM access. User can only access the objects they can view in the interface normally. Allows for setting specific permissions for the user.'); ?><br><br><b><?php echo _('Full'); ?></b> - <?php echo _('Integrated CCM access. Can access all objects with no admin features.'); ?>">help</i>
                    </td>
                    <td>
                        ...
                        <div class="well" style="margin: 0;">
                            ...
                            <table class="table table-condensed table-no-border table-bordered">
                                <thead style="border-bottom: 1px solid #EEE;">
                                    <tr>
                                        <th id="limited-access-table" colspan="3"><?php echo _("Alerting Permissions"); ?></th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr>
    ...
    <div id="formButtons" style="margin-top: 3em;">
        ...
        <button type="button" class="btn btn-sm btn-primary import icon-in-btn" name="updateButton" disabled><?php echo _('Import'); ?> <i class="material-symbols-outlined md-20 md-400">chevron_right</i></button>
    </div>
